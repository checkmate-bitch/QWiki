    ...
    $.getJSON(url, data => {
        ...
        var html = children.map( child => {
            if(child.tagName === "P")
                ...
                return `<ul class="qwiki-extension-para-graph" style="color: ${qwikiColorPalette.paraColor} !important;">${child.innerText}</ul>`;
            else return;
        }) 
            .join("")
            .replace(/\[\w+\]/g, "");
        ...
        html = `<h1 class="qwiki-extension-panel-h1"><a style="color: ${qwikiColorPalette.mainHeading} !important" title="link to Wiki" href="https://en.wikipedia.org/wiki/${val}" target="_blank">${original_val}</a></h1>` + html;    
